...
# --- 3. نظام إضافة سؤال (نظيف بالكامل مع حذف رسائل البوت) ---
...
async def start_add_question(c: types.CallbackQuery, state: FSMContext):
    ...
    # تعديل الرسالة الحالية لطلب السؤال
...
async def process_q_text(message: types.Message, state: FSMContext):
    ...
    # إرسال رسالة جديدة لطلب الجواب وحفظ الـ ID لحذفها لاحقاً
...
async def process_first_ans(message: types.Message, state: FSMContext):
    ...
    try: 
        await message.delete() # حذف رسالة المستخدم (الجواب)
        await bot.delete_message(message.chat.id, data['last_bot_msg_id']) # حذف رسالة البوت السابقة
    except: pass
...
async def process_second_ans(message: types.Message, state: FSMContext):
    ...
    await finalize_and_return(message, state, data['current_cat_id'])

# --- معالجة الحفظ بإجابة واحدة ---
...
async def finalize_no_second(c: types.CallbackQuery, state: FSMContext):
    ...
    await finalize_and_return(c.message, state, data['current_cat_id'])

# دالة الرجوع للوحة التحكم
async def finalize_and_return(message, state, cat_id):
    await state.finish()
