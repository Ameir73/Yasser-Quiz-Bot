...
def get_settings_keyboard(settings):
    t_labels = {20: "20 ثانية", 30: "30 ثانية", 60: "دقيقة"}
    keyboard = [
        [InlineKeyboardButton(f"📊 الأسئلة: {settings['num_questions']}", callback_data="none"),
         InlineKeyboardButton(f"⏳ الوقت: {t_labels.get(settings['q_time'], '30 ثانية')}", callback_data="none")],
        
        [InlineKeyboardButton(f"✅ {n}" if settings['num_questions'] == n else str(n), callback_data=f"set_num_{n}") for n in [20, 30, 40, 50]],
        
        [InlineKeyboardButton(f"⏱️ {t_labels[t]}" if settings['q_time'] == t else t_labels[t], callback_data=f"set_time_{t}") for t in [20, 30, 60]],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome_text = (
        f"👋 **أهلاً بك يا {update.effective_user.first_name} في بوت المسابقات!**\n\n"
        "• أرسل كلمة (**تحكم**) لإدارة أقسامك.\n"
        "• أرسل كلمة (**مسابقة**) لتشغيل مسابقاتك.\n\n"
        f"👑 **المطور:** [ياسر](https://t.me/Ya_79k)"
    )
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "gui_view_cats":
            ...
            keyboard = [[InlineKeyboardButton("➕ إضافة سؤال للقسم", callback_data="gui_add_q")],
                        [InlineKeyboardButton("🔙 رجوع", callback_data="gui_view_cats")]]
            await query.edit_message_text(f"⚙️ إدارة القسم المختار:", reply_markup=InlineKeyboardMarkup(keyboard))
            ...
            context.user_data['state'] = 'WAIT_Q_TEXT' # تم الإصلاح من user_id إلى user_data
            await query.edit_message_text("❓ أرسل نص السؤال الآن:")

        elif data == "setup_quiz":
            context.user_data['temp_setup'] = {'num_questions': 20, 'q_time': 30, 'timing_mode': 'السرعة', 'comp_type': 'خاصة', 'ans_type': 'مباشرة', 'selected_cats': []}
            keyboard = [[InlineKeyboardButton("⭐ اختيار الأقسام", callback_data="quiz_select_flow")], 
                        [InlineKeyboardButton("🔙 رجوع", callback_data="back_to_main")]]
            await query.edit_message_text("🏆 تهيئة مسابقة جديدة:", reply_markup=InlineKeyboardMarkup(keyboard))
            ...
            await query.edit_message_text("⚙️ إعدادات المسابقة الفنية:", reply_markup=get_settings_keyboard(context.user_data['temp_setup']), parse_mode='Markdown')
            ...
            context.user_data['temp_setup']['q_time'] = int(data.split("_")[2]) # تم إصلاح تقسيم الداتا هنا
            await query.edit_message_reply_markup(reply_markup=get_settings_keyboard(context.user_data['temp_setup']))

        elif data in ["toggle_timing", "toggle_comp"]:
            s = context.user_data['temp_setup']
            if data == "toggle_timing": s['timing_mode'] = "الوقت" if s['timing_mode'] == "السرعة" else "السرعة"
            else: s['comp_type'] = "عامة" if s['comp_type'] == "خاصة" else "خاصة"
            ...
            await query.edit_message_text("📝 أرسل اسماً لحفظ المسابقة:")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'WAIT_CAT_NAME':
        ...
        context.user_data['state'] = None
        await update.message.reply_text("🚀 تم حفظ السؤال في قسمك!")
        ...
        # 2. حفظ المسابقة وربطها
        supabase.table("active_quizzes").insert({
            "quiz_name": text, "settings_id": res_set.data[0]['id'], "created_by": user_id
        }).execute()
        context.user_data['state'] = None
        context.user_data['temp_setup'] = None
        await update.message.reply_text(f"✅ تم حفظ المسابقة باسم '{text}'! أرسل كلمة 'مسابقة' للتشغيل.")
