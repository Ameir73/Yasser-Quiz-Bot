...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 3. إدارة القسم (حذف، تغيير، إضافة أسئلة)
        ...
        keyboard = [
            [InlineKeyboardButton("🗑️ حذف القسم", callback_data=f"del_cat_{cat_id}"), InlineKeyboardButton("✏️ تغيير الاسم", callback_data=f"edit_n_{cat_id}")],
            ...
            [InlineKeyboardButton("🔙 رجوع للقائمة", callback_data="gui_view_cats")]
        ]
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard))

    # 4. حذف وتغيير الاسم (الإصلاح)
    elif data.startswith("del_cat_"):
        cat_id = data.split("_")[2]
        supabase.table("categories").delete().eq("id", cat_id).execute()
        await query.edit_message_text("🗑️ تم حذف القسم بنجاح.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 عودة للأقسام", callback_data="gui_view_cats")]]))
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'WAIT_CAT_NAME':
        ...
        await update.message.reply_text(f"✅ تم إضافة قسم '{text}' بنجاح! جاري العودة للأقسام...")
        # عودة تلقائية لقائمة الأقسام
        res = supabase.table("categories").select("*").execute()
        keyboard = [[InlineKeyboardButton(f"{c['name']}", callback_data=f"manage_cat_{c['id']}")] for c in res.data]
        ...
        await update.message.reply_text("📂 أقسامك الخاصة المتاحة:", reply_markup=InlineKeyboardMarkup(keyboard))
        ...
        await update.message.reply_text("هل تريد إضافة إجابة بديلة ثانية؟", reply_markup=InlineKeyboardMarkup(keyboard))
