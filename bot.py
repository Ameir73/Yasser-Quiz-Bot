...
# الإعدادات الجديدة تضمن الاتصال بالإصدار المستقر وتجنب v1beta القديم
ai_model = genai.GenerativeModel(
    model_name='gemini-1.5-flash',
    generation_config={
        "temperature": 0.8,
        "top_p": 0.95,
        "max_output_tokens": 70, # زيادة طفيفة لضمان جملة تلميح مفيدة
    }
)

async def get_ai_hint(question, answer):
    """دالة استخراج تلميح ذكي عبر Gemini"""
    # البرومبت (الأمر) مصمم لجعل الذكاء الاصطناعي يساعد المتسابق دون غش
    prompt = f"أنت مساعد في مسابقة ثقافية. السؤال هو: '{question}' والإجابة الصحيحة هي: '{answer}'. أعطني تلميحاً ذكياً وقصيراً جداً يساعد المتسابق على معرفة الإجابة دون ذكر حروفها أو الكلمة نفسها نهائياً."
    try:
        # تشغيل الطلب في Thread منفصل لمنع تعليق البوت أثناء التفكير
        response = await asyncio.to_thread(ai_model.generate_content, prompt)
        if response and response.text:
            return response.text.strip()
        else:
            return "فكر جيداً، الإجابة قريبة منك! 💡"
    except Exception as e:
        logging.error(f"AI Hint Error: {e}")
        return "حاول التركيز في نص السؤال! 💡"
