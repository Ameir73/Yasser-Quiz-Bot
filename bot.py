    ...
    for line in lines:
        if '+' in line:
            ...
            if len(parts) == 3:
                ...
                try:
                    # 1. جلب أو إنشاء القسم في bot_categories [cite: 2026-01-01]
                    ...
                    # 2. الإدخال في bot_questions باستخدام الأسماء التي أكدتها [cite: 2026-02-17]
                    supabase.table("bot_questions").insert({
                        "question_content": q_text,  # الاسم الصحيح [cite: 2026-02-17]
                        "correct_answer": q_ans,    # الاسم الصحيح [cite: 2026-02-17]
                        "bot_category_id": cat_id,   # العمود الرابط من المخطط [cite: 2026-02-17]
                        "created_by": str(ADMIN_ID)  # متطلب أساسي في المخطط [cite: 2026-02-17]
                    }).execute()
                    success += 1
                except Exception as e:
                    logging.error(f"Row Upload Error: {e}")
                    # إظهار سبب الخطأ لك فوراً في حال حدوثه [cite: 2026-01-01]
                    await message.answer(f"⚠️ فشل في سطر: {q_text[:15]}...\nالسبب: {str(e)}")
                    error += 1
            else: error += 1
        else: error += 1

    await message.answer(f"📊 <b>ملخص الرفع النهائي (ياسر الملك):</b>\n✅ نجاح: {success}\n❌ فشل: {error}\n\n📥 أرسل دفعة أخرى أو أرسل 'خروج'.", parse_mode="HTML")
